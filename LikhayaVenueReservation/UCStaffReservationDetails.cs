...
{
    public partial class UCStaffReservationDetails : Form
    {
        private string reservationID = "";
        private string reservationStatus = "";
        DateTime reservationDate = DateTime.Today;
        public UCStaffReservationDetails()
        {
            InitializeComponent();
        }

        public UCStaffReservationDetails(string id) : this()
        {
            reservationID = id;
        ...
        private void loadReservation(string id)
        {
            ...
            try
            {
                ...
                SqlCommand loadReservation = new SqlCommand("SELECT r.ReservationDate, r.ReservationStartTime, r.ReservationEndTime, r.ReservationStatus, v.VenueID, v.VenueName, a.accountUsername FROM Reservation r INNER JOIN Venue v ON r.VenueID = v.VenueID INNER JOIN Account a ON r.accountID = a.accountID WHERE r.ReservationID = @reservationid", conn);
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        lblUserNameDetails.Text = reader["accountUsername"].ToString();
                    }
                    reader.Close();
                }
                else
