...
{
    public partial class UCStaffReservations : UserControl
    {
        UCStaffReservationDetails viewReservation;
        public UCStaffReservations()
        {
            InitializeComponent();
            tblViewReservations.AutoGenerateColumns = true;
            loadReservationData("SELECT r.ReservationID, v.VenueName, v.VenueLocation, r.ReservationDate, r.ReservationStartTime, r.ReservationEndTime, r.ReservationCreationDate, r.ReservationStatus, a.accountUsername FROM Reservation r LEFT JOIN Venue v ON r.VenueID = v.VenueID LEFT JOIN Account a ON r.accountID = a.accountID");
        ...
        private void tblViewReservations_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                {
                    viewReservation = new UCStaffReservationDetails(reservationID);
                }
                else
                {
        ...
        {
            string query = @"SELECT r.ReservationID, v.VenueName, v.VenueLocation, r.ReservationDate, r.ReservationStartTime, r.ReservationEndTime, r.ReservationCreationDate, r.ReservationStatus, a.accountUsername FROM Reservation r LEFT JOIN Venue v ON r.VenueID = v.VenueID LEFT JOIN Account a ON r.accountI = a.accountID WHERE r.accountID = @accountid AND (v.VenueName = @searchvalue OR v.VenueLocation LIKE @searchValue OR r.ReservationStatus LIKE @searchvalue OR a.accountUsername LIKE @searchValue)";
